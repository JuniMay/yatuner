...
class LinUCB:
    ...
    def recommend(self, usr_features) -> Union[int, float]:
        assert len(usr_features) == self.dimension, "usr_features's dimension dismatch"
        ...
        pta = np.dot(xaT, theta_t) + self.alpha * np.sqrt(np.dot(np.dot(xaT, AaI_t), xa))
    ...
    def update(self, reward) -> None:
        ...
        self.AaI[self.a_max] = np.linalg.solve(self.Aa[self.a_max], np.identity(self.dimension))
